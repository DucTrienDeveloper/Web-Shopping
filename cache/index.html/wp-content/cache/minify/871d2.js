(function($) {
    ...
    jQuery(document).ready(function() {
        ...
        $('body').on('click', '.apus-woocommerce-product-gallery-thumbs .woocommerce-product-gallery__image a', function(e) { e.preventDefault(); });
        $('a[data-toggle="tab"]').on('shown.bs.tab', function(e) {
            var target = $(e.target).attr("href");
            var $slick = $("[data-carousel=slick]", target);
            if ($slick.length > 0 && $slick.hasClass('slick-initialized')) { $slick.slick('refresh'); }
        ...
        $('body').on('click', '[data-load="ajax"] a', function(e) {
            ...
            if (main.length > 0) {
                if (main.data('loaded') == false) {
                    ...
                    $.ajax({ url: yozi_ajax.ajaxurl, type: 'POST', dataType: 'html', data: { action: 'yozi_ajax_get_products', settings: main.data('settings'), tab: main.data('tab'), security: yozi_ajax.ajax_nonce } }).done(function(reponse) {
                        main.html(reponse);
                        main.parent().removeClass('loading');
                        main.parent().find('.tab-pane').removeClass('active');
                        main.addClass('active');
                        if (main.find('.slick-carousel')) { init_slick(main.find('.slick-carousel')); }
                        initProductImageLoad();
                    });
                    return true;
                } else {
        ...
        function initProductImageLoad() {
            ...
            if ($images.length) { $images.unveil(1, function() { $(this).load(function() { $(this).parents('.product-image').first().addClass('image-loaded'); }); }); }
        }
        if ($('.counterUp').length > 0) { $('.counterUp').counterUp({ delay: 10, time: 800 }); }
        jQuery('.isotope-items').each(function() {
            var $container = jQuery(this);
            setTimeout(function() { $container.isotope({ itemSelector: '.isotope-item', transformsEnabled: true, masonry: { columnWidth: $container.data('columnwidth') } }); }, 100);
        });
        ...
        setTimeout(function() { change_margin_top(); }, 50);
        ...
        $('.widget-gallery').each(function() {
            ...
            $('#' + tagID).magnificPopup({ delegate: '.popup-image-gallery', type: 'image', tLoading: 'Loading image #%curr%...', mainClass: 'mfp-img-mobile', gallery: { enabled: true, navigateByImgClick: true, preload: [0, 1] } });
        });
        ...
        $('.apus-google-map').each(function() {
            ...
            var id = $(this).attr('id');
            if ($(this).data('marker_icon')) { var marker_icon = $(this).data('marker_icon'); } else { var marker_icon = ''; }
        ...
        var mobilemenu = $("#navbar-offcanvas").mmenu({ offCanvas: true, }, { offCanvas: { pageSelector: "#wrapper-container" } });
        $('.sidebar-right, .sidebar-left').perfectScrollbar();
        $('body').on('click', '.mobile-sidebar-btn', function() {
            if ($('.sidebar-left').length > 0) { $('.sidebar-left').toggleClass('active'); } else if ($('.sidebar-right').length > 0) { $('.sidebar-right').toggleClass('active'); }
            $('.mobile-sidebar-panel-overlay').toggleClass('active');
        });
        $('body').on('click', '.mobile-sidebar-panel-overlay, .close-sidebar-btn', function() {
            if ($('.sidebar-left').length > 0) { $('.sidebar-left').removeClass('active'); } else if ($('.sidebar-right').length > 0) { $('.sidebar-right').removeClass('active'); }
            $('.mobile-sidebar-panel-overlay').removeClass('active');
        });
        $('body').on('click', '.mobile-vertical-menu-title', function() {
            $('.mobile-vertical-menu').slideToggle();
            $(this).toggleClass('active');
            if ($(this).find('i').hasClass('fa-angle-down')) { $(this).find('i').removeClass('fa-angle-down').addClass('fa-angle-up'); } else { $(this).find('i').addClass('fa-angle-down').removeClass('fa-angle-up'); }
        });
        $("#vertical-mobile-menu .icon-toggle").on('click', function() {
            $(this).parent().find('> .sub-menu').slideToggle();
            if ($(this).find('i').hasClass('fa-plus')) { $(this).find('i').removeClass('fa-plus').addClass('fa-minus'); } else { $(this).find('i').removeClass('fa-minus').addClass('fa-plus'); }
            return false;
        });
        $('body').on('click', '.footer-search-btn', function() { $('.footer-search-mobile').toggleClass('active'); });
        $('body').on('click', '.more', function() { $('.wrapper-morelink').toggleClass('active'); });
    });
    $(document.body).on('click', '.nav [data-toggle="dropdown"]', function() { if (this.href && this.href != '#') { window.location.href = this.href; } });
    $(document).ready(function() { $('body').on('click', ".vertical-wrapper .title-vertical", function() { $(".vertical-wrapper .content-vertical").slideToggle(300); }); });
})(jQuery)
